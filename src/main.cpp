...
#if defined(NDEBUG)
# error "Ljcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x20da70853ee182fd73a82b1223374bf1f70ee754511f76f4affe5d796f23bbbd");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Ljcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Ljcoin Signed Message:\n";
...
{
    // Ljcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Ljcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Ljcoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Ljcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Ljcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Ljcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x3f82793b5f43682f295e2727c5953353c41a5455449196ae7c581438cafd8e45");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "I am only interested in mice";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04e72b5b1747ce7b02fffb5b640f76270df7123081179873641c87bf6a7db5e51d4ac882197d68d92091427d06776646d08fd79ddca792e46af518786feafccaab") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1524471847;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1817758;

        if (fTestNet)
        {
            block.nTime    = 1524471813;
            block.nNonce   = 548237;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x0e4bc0860060a1457697f6d581ade5fae5ed4715b1a16bc4fafff3806ec1ecdf"));
...
unsigned char pchMessageStart[4] = { 0x47, 0x77, 0x22, 0x13 }; // Ljcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("LjcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("LjcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static LjcoinMiner(CWallet *pwallet)
{
    printf("LjcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("ljcoin-miner");
    ...
    try { loop {
        ...
        printf("Running LjcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&LjcoinMiner, pwallet));
}
